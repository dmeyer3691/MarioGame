...
namespace TeamYoshi
{
    static public class MarioLevelLoader
    {
        ...
        static public MarioLevel LoadLevelFromFile(String filename)
        {
        ...
        static private MarioLevel LoadLevel(StreamReader stream)
        {
            ...
            Camera cam = new Camera();
                ...
                while ((tag = stream.ReadLine()) != null && !tag.Equals("</level>"))
                {
                    ...
                    if (isMario) mario = new Mario(xpos.Value, ypos.Value, cam);
                    else
                    {
                        switch (type.Trim('"'))
                        {
                ...
                cam.Init(mario);
                return new MarioLevel(items,enemies,blocks,mario,cam);
        }
    }
}
